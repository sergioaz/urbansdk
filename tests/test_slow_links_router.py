...
async def setup_database():
    ...
    if not database.is_connected:
        print ("in setup_database waiting for connect")
        await database.connect()
    yield
    # Keep connection for other tests, disconnect will be handled by app lifespan
    print("in setup_database waiting for disconnect")
    await database.disconnect()


@pytest.mark.asyncio
async def test_get_slow_links_valid_data():
    # Ensure database is connected
    if not database.is_connected:
        await database.connect()

    print ("in test_get_slow_links_valid_data")
    period = "AM Peak"
    threshold = 25.0
    min_days = 7
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get("/slow_links/", params={"period": period, "threshold": threshold, "min_days": min_days})
        ...
        if response.status_code == 200:
            ...
            for link in data:
                ...
                assert isinstance(link["overall_average_speed"], (int, float))
...
@pytest.mark.asyncio
async def test_get_slow_links_invalid_period():
    period = "Invalid"
    threshold = 25.0
    min_days = 7
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
...
@pytest.mark.asyncio
async def test_get_slow_links_missing_params():
    period = "Invalid"
    threshold = 25.0
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        response = await ac.get("/slow_links/", params={"period": period, "threshold": threshold})
...
async def test_get_slow_links_with_mock_data(mock_service):
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        mock_service.assert_called_once_with(3, 25.0, 7)  # day_number=3, period_number=3
