import logging
from fastapi import APIRouter, Query, HTTPException
from app.models.aggregate import AggregateResponse
from app.services.aggregate import get_average_speed_by_day_period

router = APIRouter()

logger = logging.getLogger(__name__)

@router.get("/aggregates/", response_model=AggregateResponse)
async def get_aggregated_speed(
    day: int = Query(..., description="Day of week (1-7, where 1=Monday)", ge=1, le=7),
    period: int = Query(..., description="Time period identifier", ge=1)
):
    """
    Get aggregated average speed for the given day and time period.
    
    Args:
        day: Day of week (1-7, where 1=Monday)
        period: Time period identifier
        
    Returns:
        AggregateResponse: Aggregated data with average speed
        
    Example API Calls:
        GET /aggregates/?day=2&period=4
        GET /aggregates/?day=1&period=7
        GET /aggregates/?day=5&period=1
        
    Example Response:
        {
            "day_of_week": 2,
            "period": 4,
            "average_speed": 42.75
        }
        
    Day Mapping:
        1 = Monday
        2 = Tuesday
        3 = Wednesday
        4 = Thursday
        5 = Friday
        6 = Saturday
        7 = Sunday
    """
    try:
        # Call the service function
        result = await get_average_speed_by_day_period(day, period)
        
        # Check if data was found
        if result["average_speed"] == 0.0:
            raise HTTPException(
                status_code=404, 
                detail=f"No data found for day {day} and period {period}"
            )
        
        return AggregateResponse(**result)
        
    except HTTPException:
        # Re-raise HTTP exceptions
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")
